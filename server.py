#!/usr/bin/env python3
"""
test izzy 4 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('test-izzy-4_mcp')
...
logger.info("="*80)
logger.info(f"test izzy 4 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://petstore.swagger.io/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("test izzy 4 MCP Server", host="0.0.0.0")
...
# - TEST_IZZY_4_API_KEY: Server's internal API key for payment mode
#
...
async def Finds Pets by status(
    context: Context,
    ...
    Example Usage:
        await Finds Pets by status(status=None)
    ...
    try:
        ...
        logger.error(f"Error in Finds Pets by status: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-izzy-4-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "test-izzy-4-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting test izzy 4 MCP Server")
    logger.info("="*80)
